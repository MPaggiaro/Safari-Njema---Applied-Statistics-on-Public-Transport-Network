...
baseday = 69 #71 scelta ad-hoc (merc, 13 marzo 2019) -> NB: poi nel ciclo partiamo da baseday+1 !!!
# NB: se uso dataframe df_trips togliendo i viaggi di meno di 2 km, metto 
# baseday = 72
{
pval_mtrx <- matrix(0, N,  N) #nizializza matrice p_value

# definisci distribuzione viaggi da zona_3 a zona_i per ogni giorno
distrib_per_day  <- count( df_trips_district[df_trips_district$dep_ID == 3 & 
                                            df_trips_district$day == baseday + N, 
                                            grep("arr_ID", colnames(df_trips_district))] )[,2]
...
for ( i in seq(1,N-1) ){
  ...
  distrib_per_day <- cbind( distrib_per_day,  count(df_dist_1)[,2] )
  for ( j in seq(i+1,N) ){
...
# rearranging in right order
distrib_per_day <- cbind( distrib_per_day[,2:dim(distrib_per_day)[2]] , distrib_per_day[,1])

print(pval_mtrx)
print(distrib_per_day)
}
